...
class PCPanel(FilePanel):
    ...
    def draw(self, is_active: bool):
        ...
        for i in range(visible_height):
            ...
            attr = curses.A_REVERSE if list_index == self.selected_index else curses.A_NORMAL
            if is_dir:
                attr |= curses.A_BOLD
...
class CoCoCommander:
    ...
    def show_message(self, message: str, error: bool = False):
        ...
        msg_win.addstr(height - 2, 2, "Press any key to continue...", curses.A_DIM)
        msg_win.refresh()
        msg_win.getch()
        ...
        if self.active_panel == 0:
            ...
            try:
                if self.dsk_panel.dsk.upload_from_pc(str(file_path), dsk_name,
                    ...
                    self.show_message(f"File uploaded successfully:\n{dsk_name}")
                else:
            ...
            try:
                if self.dsk_panel.dsk.copy_to_pc(full_name, str(output_path)):
                    ...
                    self.show_message(f"File downloaded successfully:\n{pc_name}")
                else:
    ...
    def input_dialog(self, title: str, prompt: str, default: str = "") -> Optional[str]:
        ...
        input_win.addstr(input_y, 4, default)
        input_win.refresh()

        # Get input
        input_win.move(input_y, 4 + len(default))
        result = input_win.getstr(input_y, 4, max_width - 8).decode('utf-8')
